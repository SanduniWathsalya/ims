...
const Editaddress = () => {
  ...
  const [errors, setErrors] = useState({ address: "", province: "" });

  const handleSubmit = (event) => {
      event.preventDefault();
      let formValid = true;
      let newErrors = { address: "", province: "" };

      if (address.trim() === "") {
          newErrors.address = "Please fill this section first";
          formValid = false;
      }
      if (province.trim() === "") {
          newErrors.province = "Please fill this section first";
          formValid = false;
      }
...
<div class="bg-white    w-full overflow-hidden">
 <div className="grid md:grid-cols-4 border-b border-yellow-300">
            <div className="p-6 border-r border-yellow-300 text-center relative opacity-50">
                ...
                <h3 className="text-sm font-semibold text-green-500">Contact Details</h3>
                        ...
                        <div className="p-6 border-r border-yellow-300  text-center relative ">
                <div className="flex items-center justify-center mb-4">
                    <div className="w-12 h-12 rounded-full bg-yellow-500 text-white flex items-center justify-center font-bold text-lg relative z-10 ">
        ...
        <div class="p-12">
            <h2 class="text-3xl font-bold mb-8 text-yellow-600">Address</h2>
            
            <form onSubmit={handleSubmit}>
            <div class="grid md:grid-cols-2 gap-6">
                <div>
                    <label className="block mb-3 text-sm font-medium text-yellow-500">Address</label>
                    <div class="relative">
                    <input 
                        type="text" 
                        placeholder="12th lane,Koswatta"
                        value={address} 
                        onChange={(e) => setAddress(e.target.value)}
                        className={`w-full px-4 py-3 border-2 rounded-xl text-black bg-transparent focus:outline-none focus:ring-2 pl-10
                            ${errors.address ? "border-red-500 focus:ring-red-500" : "border-yellow-400 focus:ring-yellow-500"}`}
                    />
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 text-yellow-500 absolute left-3 top-1/2 transform -translate-y-1/2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z" />
                        </svg>
                    </div>
                </div>
                <div>
                    <label className="block mb-3 text-sm font-medium text-yellow-500">Province</label>
                    <div class="relative">
                    <input 
                        type="text" 
                        placeholder="colombo"
                        ...
                        className={`w-full px-4 py-3 border-2 rounded-xl text-black bg-transparent focus:outline-none focus:ring-2 pl-10
                            ${errors.province ? "border-red-500 focus:ring-red-500" : "border-yellow-400 focus:ring-yellow-500"}`}
                    />
                        
                        <svg xmlns="http://www.w3.org/2000/svg" 
                        className="h-5 w-5 text-yellow-500 absolute left-3 top-1/2 transform -translate-y-1/2" 
     fill="none" viewBox="0 0 24 24" stroke="currentColor">
    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" 
          d="M12 2C8.13 2 5 5.13 5 8.5c0 3.34 4 8.5 7 11 3-2.5 7-7.66 7-11 0-3.37-3.13-6.5-7-6.5zM12 11a3 3 0 100-6 3 3 0 000 6z" />
</svg>
            ...
            <div className="flex justify-between items-center mt-10">
                    <p className="text-sm text-yellow-400 transform transition-transform duration-200 hover:scale-110 hover:text-yellow-600 "><Link href="/Contactdetails"> Step 3 of 4</Link></p>
                    <Link href="/Confirmation">

                    <button type="submit" className="px-8 py-3 bg-yellow-600 text-white rounded-xl hover:bg-yellow-800 
